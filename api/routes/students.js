...
function add(req, res) {
    db.query('INSERT INTO students (name, user_id) VALUES ($1, $2)', [req.body.name, req.body.userId])
        .then(response => {
...
function updateById(req, res) {
    db.query('UPDATE students SET name = $1 WHERE id = $2', [req.body.name, req.params.id])
        .then(response => {
            console.log(response)
            res.send({ success: true })
        })
